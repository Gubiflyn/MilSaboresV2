...
describe("Productos", () => {
  it("TPTS-Pages 27: Muestra productos correctamente al cargar la página", async () => {
    renderPage();
    expect(
  ...
  it("TPTS-Pages 28: Redirecciona correctamente al hacer click en una torta", async () => {
    renderPage();
  ...
  it("TPTS-Pages 29: Botón 'Agregar al carrito' está presente y visible", async () => {
    renderPage();
  ...
  it("TPTS-Pages 30: Filtra productos correctamente por categoría", async () => {
    renderPage();
  ...
  it("TPTS-Pages 31: Muestra precios en formato correcto (CLP)", async () => {
    renderPage();
    const tituloTorta = await screen.findByText("Torta Cuadrada de Chocolate");
    const cardTorta = tituloTorta.closest(".card") || tituloTorta.closest("*");
